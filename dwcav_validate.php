...
function dwcav_meta_xml($dir, $ok_files) {
  ...
  //Validate attributes
  dwcav_xml_attributes($meta_xml->core->attributes(), 'meta.xml');
...
function dwcav_xml_attributes_ignoreHEaderLines($check, $file) {
  if(!is_int($check)) {
  	dwcav_error('error', $file, "ignoreHeaderLines is not an integer for $file in meta.xml");
  	return;
  }
  if ($check > 1) {
  	dwcav_error('info', $file, "ignoreHEaderLines seems rather large, are you sure it's correct?");
  }
}

function dwcav_xml_attributes_rowType($check, $file) {
  if (!filter_var($check, FILTER_VALIDATE_URL)) {
  	dwcav_error('error', $file, "rowType for $file in meta.xml is not a valid URL");
  }	
}
...
function dwcav_xml_attributes_linesTerminatedBy($check, $file) {
  ...
  if (!in_array($check, $normal_values)) {
  	dwcav_error('info', $file, "Non-standard value for linesTerminatedBy in $file");
  }
}
...
function dwcav_xml_attributes_fieldsEnclosedBy($check, $file) {
  ...
  if (!in_array($check, $normal_values)) {
  	dwcav_error('info', $file, "Non-standard value for fieldsEnclosedBy in $file");
  }
}
...
function dwcav_xml_attributes_fieldsTerminatedBy($check, $file) {
  ...
  if (!in_array($check, $normal_values)) {
  	dwcav_error('info', $file, "Non-standard value for fieldsTerminatedBy in $file");
  }
}
